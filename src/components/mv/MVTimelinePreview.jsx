...
function MVTimelinePreview({ result, onCopy, t }) {
    ...
    return (
        <div className="mt-6 p-4 md:p-6 glass rounded-xl border border-white/10">
                        ...
                        <div className="space-y-2">
                            {timeline.map((scene, i) => (
                                <div key={i} className="p-3 rounded-lg bg-white/5 border border-white/10 flex items-center gap-3">
                                    <div className="w-10 h-10 md:w-8 md:h-8 rounded-lg bg-gradient-to-br from-sky-500 to-cyan-500 flex items-center justify-center text-sm md:text-xs font-bold text-white flex-shrink-0">
                                        {scene.sceneId || i + 1}
                                    </div>
                                    ...
                                    {scene.transition && (
                                        <div className="text-[10px] px-2 py-1 rounded bg-sky-500/10 text-sky-300 border border-sky-500/20 flex-shrink-0">
                                            {scene.transition}
                                        </div>
                                    )}
                                </div>
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {effects.map((effect, i) => (
                                <span key={i} className="text-xs px-2 py-1 rounded bg-sky-500/10 text-sky-300 border border-sky-500/20">
                                    {effect}
                                </span>
                            ))}
                        </div>
